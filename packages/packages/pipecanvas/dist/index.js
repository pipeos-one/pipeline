    ...
    function show(x) {
      ...
      switch (Object.prototype.toString.call(x)) {
        ...
        case '[object Array]':
          ...
          try {
            return '[' + x.map(show).concat(Object.keys(x).sort().filter(function (k) {
              return !/^\d+$/.test(k);
            }).map(entry(x))).join(', ') + ']';
          } finally {
            seen.pop();
        ...
        case '[object Object]':
          ...
          try {
            return $$show in x && (x.constructor == null || x.constructor.prototype !== x) ? x[$$show]() : '{' + Object.keys(x).sort().map(entry(x)).join(', ') + '}';
          } finally {
            seen.pop();
          }
...
var sanctuaryTypeClasses = createCommonjsModule(function (module) {
  ...
  (function (f) {
    ...
    TypeClass['@@type'] = 'sanctuary-type-classes/TypeClass@1'; //  data Location = Constructor | Value
    //  Constructor :: Location

    var Constructor = 'Constructor'; //  Value :: Location

    var Value = 'Value'; //  _funcPath :: (Boolean, Array String, a) -> Nullable Function
    ...
    var Category = $('Category', [Semigroupoid], {
      id: Constructor
    }); //# Semigroup :: TypeClass
    //.
    ...
    var Bifunctor = $('Bifunctor', [Functor], {
      bimap: Value
    ...
    var Profunctor = $('Profunctor', [Functor], {
      promap: Value
    }); //# Apply :: TypeClass
    //.
    ...
    var Applicative = $('Applicative', [Apply], {
      of: Constructor
    }); //# Chain :: TypeClass
    //.
    ...
    var ChainRec = $('ChainRec', [Chain], {
      chainRec: Constructor
    }); //# Monad :: TypeClass
    //.
    ...
    var Monad = $('Monad', [Applicative, Chain], {}); //# Alt :: TypeClass
    //.
    ...
    var Alternative = $('Alternative', [Applicative, Plus], {}); //# Foldable :: TypeClass
    //.
    ...
    var Contravariant = $('Contravariant', [], {
      contramap: Value
    ...
    function Number$prototype$lte(other) {
      return typeof this === 'object' ? lte(this.valueOf(), other.valueOf()) : isNaN(this) || this <= other;
    ...
    var Array$prototype$alt = Array$prototype$concat; //  Array$prototype$reduce :: Array a ~> ((b, a) -> b, b) -> b
    ...
    function Object$prototype$equals(other) {
      ...
      return equals(keys, sortedKeys(other)) && keys.every(function (k) {
    ...
    var Object$prototype$alt = Object$prototype$concat; //  Object$prototype$reduce :: StrMap a ~> ((b, a) -> b, b) -> b
    ...
    var equals = function () {
      ...
      return function equals(x, y) {
        if (!sameType(x, y)) return false; //  This algorithm for comparing circular data structures was
...
var sanctuaryEither = createCommonjsModule(function (module) {
  ...
  (function (f) {
    /* istanbul ignore else */
    {
      module.exports = f(util, sanctuaryShow, sanctuaryTypeClasses);
    }
    ...
    if (typeof __doctest !== 'undefined') {
      ...
      var S = function () {
        ...
        var EitherType = $.BinaryType('sanctuary-either/Either')('')(function (x) {
        ...
        var env = Z.concat(S.env, [$.TypeClass, EitherType($.Unknown)($.Unknown)]);
        return S.create({
          checkTypes: true,
          env: env
        });
    ...
    Either['@@type'] = 'sanctuary-either/Either@1'; //# Either.fantasy-land/of :: b -> Either a b
    //.
...
var sanctuaryDef = createCommonjsModule(function (module) {
  ...
  (function (f) {
    /* istanbul ignore else */
    {
      module.exports = f(util, sanctuaryEither, sanctuaryShow, sanctuaryTypeClasses, sanctuaryTypeIdentifiers);
    }
    ...
    /* istanbul ignore next */
    'inspect'; //  Left :: a -> Either a b

    var Left = Either.Left; //  Right :: b -> Either a b

    var Right = Either.Right; //  B :: (b -> c) -> (a -> b) -> a -> c
    ...
    var _ = r(' '); //  sortedKeys :: Object -> Array String
    ...
    function toArray(foldable) {
      return Array.isArray(foldable) ? foldable : Z.reduce(function (xs, x) {
        xs.push(x);
        return xs;
      }, [], foldable);
    ...
    function toMarkdownList(empty, s, f, xs) {
      return isEmpty(xs) ? empty : Z.reduce(function (s, x) {
        return s + '  - ' + f(x) + '\n';
      }, s, xs);
    ...
    var q = wrap('\u2018')('\u2019'); //  stripNamespace :: TypeClass -> String
    ...
    var Type$prototype = {
      ...
      'fantasy-land/equals': function (other) {
        return Z.equals(this.type, other.type) && Z.equals(this.name, other.name) && Z.equals(this.url, other.url) && Z.equals(this.supertypes, other.supertypes) && Z.equals(this.keys, other.keys) && Z.equals(this.types, other.types);
      },
      '@@show': function () {
    ...
    function _Type(type, // :: String
    name, // :: String
    url, // :: String
    arity, // :: NonNegativeInteger
    format, // :: Nullable ((String -> String, String -> String -> String) -> String)
    ...
    tuples // :: Array (Array3 String (a -> Array b) Type)
      ...
      t.format = format || function (outer, inner) {
        return Z.reduce(function (s, tuple) {
          return s + outer(' ') + when(tuple[2].arity > 0)(parenthesize(outer))(inner(tuple[0])(show(tuple[2])));
        }, outer(name), tuples);
      };

      t.keys = tuples.map(function (tuple) {
        return tuple[0];
      });
      t.name = name;
    ...
    var UNARY = 'UNARY';
    var UNKNOWN = 'UNKNOWN';
    var VARIABLE = 'VARIABLE'; //  Inconsistent :: Type

    var Inconsistent = _Type(INCONSISTENT, '', '', 0, always2('???'), [], K(K(false)), []); //  NoArguments :: Type


    var NoArguments = _Type(NO_ARGUMENTS, '', '', 0, always2('()'), [], K(K(true)), []); //  arityGte :: NonNegativeInteger -> Type -> Boolean
    ...
    function functionUrl(name) {
      ...
      return 'https://github.com/sanctuary-js/sanctuary-def/tree/v' + version + '#' + name;
    }

    var NullaryTypeWithUrl = Z.ap(NullaryType, functionUrl);
    var UnaryTypeWithUrl = Z.ap(UnaryType, functionUrl);
    var BinaryTypeWithUrl = Z.ap(BinaryType, functionUrl); //. ### Types
    //.
    ...
    var Unknown = _Type(UNKNOWN, '', '', 0, always2('Unknown'), [], K(K(true)), []); //# Any :: Type
    //.
    //. Type comprising every JavaScript value.


    var Any = NullaryTypeWithUrl('Any')([])(K(true)); //# AnyFunction :: Type
    //.
    //. Type comprising every Function value.

    var AnyFunction = NullaryTypeWithUrl('Function')([])(typeofEq('function')); //# Arguments :: Type
    //.
    //. Type comprising every [`arguments`][arguments] object.

    var Arguments = NullaryTypeWithUrl('Arguments')([])(typeEq('Arguments')); //# Array :: Type -> Type
    //.
    //. Constructor for homogeneous Array types.

    var Array_ = UnaryTypeWithUrl('Array')([])(typeEq('Array'))(I); //# Array0 :: Type
    //.
    //. Type whose sole member is `[]`.

    var Array0 = NullaryTypeWithUrl('Array0')([Array_(Unknown)])(sizeEq(0)); //# Array1 :: Type -> Type
    //.
    //. Constructor for singleton Array types.

    var Array1 = UnaryTypeWithUrl('Array1')([Array_(Unknown)])(sizeEq(1))(I); //# Array2 :: Type -> Type -> Type
    //.
    //. Constructor for heterogeneous Array types of length 2. `['foo', true]` is
    //. a member of `Array2 String Boolean`.

    var Array2 = BinaryTypeWithUrl('Array2')([Array_(Unknown)])(sizeEq(2))(function (array2) {
      return [array2[0]];
    })(function (array2) {
      return [array2[1]];
    }); //# Boolean :: Type
    ...
    var Boolean_ = NullaryTypeWithUrl('Boolean')([])(typeofEq('boolean')); //# Date :: Type
    //.
    //. Type comprising every Date value.

    var Date_ = NullaryTypeWithUrl('Date')([])(typeEq('Date')); //# ValidDate :: Type
    //.
    //. Type comprising every [`Date`][] value except `new Date (NaN)`.

    var ValidDate = NullaryTypeWithUrl('ValidDate')([Date_])(B(complement(isNaN))(Number)); //# Descending :: Type -> Type
    //.
    //. [Descending][] type constructor.

    var Descending = UnaryTypeWithUrl('Descending')([])(typeEq('sanctuary-descending/Descending@1'))(I); //# Either :: Type -> Type -> Type
    //.
    //. [Either][] type constructor.

    var Either_ = BinaryTypeWithUrl('Either')([])(typeEq('sanctuary-either/Either@1'))(function (either) {
    ...
    var Error_ = NullaryTypeWithUrl('Error')([])(typeEq('Error')); //# Fn :: Type -> Type -> Type
    //.
    ...
    function Function_(types) {
      ...
      function format(outer, inner) {
        return when(tuples.length !== 2)(parenthesize(outer))(joinWith(outer(', '), Z.map(function (tuple) {
          return when(tuple[2].type === FUNCTION)(parenthesize(outer))(inner(tuple[0])(show(tuple[2])));
        }, init(tuples)))) + outer(' -> ') + inner(last(tuples)[0])(show(last(tuples)[2]));
      }

      return _Type(FUNCTION, '', '', types.length, format, [AnyFunction], K(K(true)), tuples);
    } //# HtmlElement :: Type
    //.
    //. Type comprising every [HTML element][].


    var HtmlElement = NullaryTypeWithUrl('HtmlElement')([])(function (x) {
      return /^\[object HTML.+Element\]$/.test(toString.call(x));
    }); //# Identity :: Type -> Type
    //.
    //. [Identity][] type constructor.

    var Identity = UnaryTypeWithUrl('Identity')([])(typeEq('sanctuary-identity/Identity@1'))(I); //# Maybe :: Type -> Type
    //.
    //. [Maybe][] type constructor.

    var Maybe = UnaryTypeWithUrl('Maybe')([])(typeEq('sanctuary-maybe/Maybe@1'))(I); //# NonEmpty :: Type -> Type
    //.
    ...
    var NonEmpty = UnaryTypeWithUrl('NonEmpty')([])(function (x) {
      return Z.Monoid.test(x) && Z.Setoid.test(x) && !Z.equals(x, Z.empty(x.constructor));
    })(function (monoid) {
      return [monoid];
    }); //# Null :: Type
    //.
    ...
    var Null = NullaryTypeWithUrl('Null')([])(typeEq('Null')); //# Nullable :: Type -> Type
    //.
    //. Constructor for types that include `null` as a member.

    var Nullable = UnaryTypeWithUrl('Nullable')([])(K(true))(function (nullable) {
    ...
    var PositiveNumber = NullaryTypeWithUrl('PositiveNumber')([Number_])(positive); //# NegativeNumber :: Type
    //.
    //. Type comprising every [`Number`][] value less than zero.

    var NegativeNumber = NullaryTypeWithUrl('NegativeNumber')([Number_])(negative); //# ValidNumber :: Type
    //.
    //. Type comprising every [`Number`][] value except `NaN`.

    var ValidNumber = NullaryTypeWithUrl('ValidNumber')([Number_])(complement(isNaN)); //# NonZeroValidNumber :: Type
    //.
    //. Type comprising every [`ValidNumber`][] value except `0` and `-0`.

    var NonZeroValidNumber = NullaryTypeWithUrl('NonZeroValidNumber')([ValidNumber])(nonZero); //# FiniteNumber :: Type
    //.
    //. Type comprising every [`ValidNumber`][] value except `Infinity` and
    //. `-Infinity`.

    var FiniteNumber = NullaryTypeWithUrl('FiniteNumber')([ValidNumber])(isFinite); //# NonZeroFiniteNumber :: Type
    //.
    //. Type comprising every [`FiniteNumber`][] value except `0` and `-0`.

    var NonZeroFiniteNumber = NullaryTypeWithUrl('NonZeroFiniteNumber')([FiniteNumber])(nonZero); //# PositiveFiniteNumber :: Type
    //.
    //. Type comprising every [`FiniteNumber`][] value greater than zero.

    var PositiveFiniteNumber = NullaryTypeWithUrl('PositiveFiniteNumber')([FiniteNumber])(positive); //# NegativeFiniteNumber :: Type
    //.
    //. Type comprising every [`FiniteNumber`][] value less than zero.

    var NegativeFiniteNumber = NullaryTypeWithUrl('NegativeFiniteNumber')([FiniteNumber])(negative); //# Integer :: Type
    //.
    //. Type comprising every integer in the range
    //. [[`Number.MIN_SAFE_INTEGER`][min] .. [`Number.MAX_SAFE_INTEGER`][max]].

    var Integer = NullaryTypeWithUrl('Integer')([ValidNumber])(function (x) {
      return Math.floor(x) === x && x >= MIN_SAFE_INTEGER && x <= MAX_SAFE_INTEGER;
    }); //# NonZeroInteger :: Type
    //.
    //. Type comprising every [`Integer`][] value except `0` and `-0`.

    var NonZeroInteger = NullaryTypeWithUrl('NonZeroInteger')([Integer])(nonZero); //# NonNegativeInteger :: Type
    //.
    ...
    var NonNegativeInteger = NullaryTypeWithUrl('NonNegativeInteger')([Integer])(nonNegative); //# PositiveInteger :: Type
    //.
    //. Type comprising every [`Integer`][] value greater than zero.

    var PositiveInteger = NullaryTypeWithUrl('PositiveInteger')([Integer])(positive); //# NegativeInteger :: Type
    //.
    //. Type comprising every [`Integer`][] value less than zero.

    var NegativeInteger = NullaryTypeWithUrl('NegativeInteger')([Integer])(negative); //# Object :: Type
    //.
    ...
    var Object_ = NullaryTypeWithUrl('Object')([])(typeEq('Object')); //# Pair :: Type -> Type -> Type
    //.
    //. [Pair][] type constructor.

    var Pair = BinaryTypeWithUrl('Pair')([])(typeEq('sanctuary-pair/Pair@1'))(function (pair) {
      return [pair.fst];
    })(function (pair) {
      return [pair.snd];
    }); //# RegExp :: Type
    ...
    var RegExp_ = NullaryTypeWithUrl('RegExp')([])(typeEq('RegExp')); //# GlobalRegExp :: Type
    //.
    //. Type comprising every [`RegExp`][] value whose `global` flag is `true`.
    //.
    //. See also [`NonGlobalRegExp`][].

    var GlobalRegExp = NullaryTypeWithUrl('GlobalRegExp')([RegExp_])(prop('global')); //# NonGlobalRegExp :: Type
    //.
    //. Type comprising every [`RegExp`][] value whose `global` flag is `false`.
    //.
    //. See also [`GlobalRegExp`][].

    var NonGlobalRegExp = NullaryTypeWithUrl('NonGlobalRegExp')([RegExp_])(complement(prop('global'))); //# StrMap :: Type -> Type
    //.
    ...
    var StrMap = UnaryTypeWithUrl('StrMap')([Object_])(K(true))(I); //# String :: Type
    //.
    //. Type comprising every primitive String value.

    var String_ = NullaryTypeWithUrl('String')([])(typeofEq('string')); //# RegexFlags :: Type
    //.
    ...
    var RegexFlags = NullaryTypeWithUrl('RegexFlags')([String_])(function (s) {
      return /^g?i?m?$/.test(s);
    }); //# Symbol :: Type
    //.
    //. Type comprising every Symbol value.

    var Symbol_ = NullaryTypeWithUrl('Symbol')([])(typeofEq('symbol')); //# Type :: Type
    //.
    //. Type comprising every `Type` value.

    var Type = NullaryTypeWithUrl('Type')([])(typeEq('sanctuary-def/Type@1')); //# TypeClass :: Type
    //.
    //. Type comprising every [`TypeClass`][] value.

    var TypeClass = NullaryTypeWithUrl('TypeClass')([])(typeEq('sanctuary-type-classes/TypeClass@1')); //# Undefined :: Type
    //.
    //. Type whose sole member is `undefined`.

    var Undefined = NullaryTypeWithUrl('Undefined')([])(typeEq('Undefined')); //# env :: Array Type
    //.
    //. An array of [types][]:
    //.
    ...
    var env = [AnyFunction, Arguments, Array_(Unknown), Array2(Unknown)(Unknown), Boolean_, Date_, Descending(Unknown), Either_(Unknown)(Unknown), Error_, Fn(Unknown)(Unknown), HtmlElement, Identity(Unknown), Maybe(Unknown), Null, Number_, Object_, Pair(Unknown)(Unknown), RegExp_, StrMap(Unknown), String_, Symbol_, Type, TypeClass, Undefined]; //  Unchecked :: String -> Type
    ...
    var production = typeof process !== 'undefined' &&
    /* global process:false */
    process != null && process.env != null && process.env.NODE_ENV === 'production'; //  numbers :: Array String

    var numbers = ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine']; //  numArgs :: Integer -> String

    function numArgs(n) {
      return (n < numbers.length ? numbers[n] : show(n)) + ' ' + (n === 1 ? 'argument' : 'arguments');
    } //  expandUnknown
    //  :: Array Type
    //  -> Array Object
    //  -> Any
        ...
        return function (value) {
          return function (extractor) {
            return function (type) {
              return type.type === UNKNOWN ? _determineActualTypes(env, seen, extractor(value)) : [type];
            };
          };
        };
      };
    ...
    function _determineActualTypes(env, // :: Array Type
    seen, // :: Array Object
    values // :: Array Any
    ) {
      ...
      function refine(types, value) {
        ...
        return Z.chain(function (t) {
          return t.validate(env)(value).isLeft ? [] : t.type === UNARY ? Z.map(fromUnaryType(t), expandUnknown2(t.extractors.$1)(t.types.$1)) : t.type === BINARY ? Z.lift2(fromBinaryType(t), expandUnknown2(t.extractors.$1)(t.types.$1), expandUnknown2(t.extractors.$2)(t.types.$2)) : // else
          [t];
        }, types);
      }

      return isEmpty(values) ? [Unknown] : or(Z.reduce(refine, env, values), [Inconsistent]);
    } //  isConsistent :: Type -> Boolean


    function isConsistent(t) {
      return t.type === UNARY ? isConsistent(t.types.$1) : t.type === BINARY ? isConsistent(t.types.$1) && isConsistent(t.types.$2) :
      /* else */
      t.type !== INCONSISTENT;
    ...
    function determineActualTypesStrict(env, values) {
      return Z.filter(isConsistent, _determineActualTypes(env, [], values));
    ...
    function updateTypeVarMap(env, // :: Array Type
    ...
    values // :: Array Any
      ...
      var expandUnknownStrict = B(B(B(filter(isConsistent))))(expandUnknown(env)([]));
      values.forEach(function (value) {
        ...
        $typeVarMap[typeVar.name].types = Z.chain(function (t) {
          return !isValid(t)(value) ? [] : typeVar.arity === 0 && t.type === UNARY ? Z.map(fromUnaryType(t), expandUnknownStrict2(t.extractors.$1)(t.types.$1)) : typeVar.arity === 0 && t.type === BINARY ? Z.lift2(fromBinaryType(t), expandUnknownStrict2(t.extractors.$1)(t.types.$1), expandUnknownStrict2(t.extractors.$2)(t.types.$2)) : // else
          [t];
    ...
    function satisfactoryTypes(env, // :: Array Type
    typeInfo, // :: TypeInfo
    typeVarMap, // :: TypeVarMap
    expType, // :: Type
    index, // :: Integer
    ...
    values // :: Array Any
      ...
      for (var idx = 0; idx < values.length; idx += 1) {
        ...
        if (result.isLeft) {
          return Left(function () {
            return invalidValue(env, typeInfo, index, Z.concat(propPath, result.value.propPath), result.value.value);
          });
        }
      }
      ...
      switch (expType.type) {
        case VARIABLE:
            ...
            for (idx = 0; idx < values.length; idx += 1) {
              for (var idx2 = 0; idx2 < typeClasses.length; idx2 += 1) {
                if (!typeClasses[idx2].test(values[idx])) {
                  return Left(function () {
                    return typeClassConstraintViolation(env, typeInfo, typeClasses[idx2], index, propPath, values[idx]);
                  });
                }
              }
            }
          ...
          var typeVarMap$ = updateTypeVarMap(env, typeVarMap, expType, index, propPath, values);
          var okTypes = typeVarMap$[typeVarName].types;
          return isEmpty(okTypes) ? Left(function () {
            return typeVarConstraintViolation(env, typeInfo, index, propPath, typeVarMap$[typeVarName].valuesByPath);
            ...
            return Z.chain(function (r) {
              ...
              return expType.keys.reduce(function (e, k, idx) {
                ...
                return Z.reduce(function (e, x) {
                  return Z.chain(function (r) {
                    return recur(env, typeInfo, r.typeVarMap, expType.types[k], index, Z.concat(propPath, [k]), [x]);
                  }, e);
        ...
        case UNARY:
          return Z.map(function (result) {
            return {
              ...
              types: Z.map(fromUnaryType(expType), or(result.types, [expType.types.$1]))
            };
          }, recur(env, typeInfo, typeVarMap, expType.types.$1, index, Z.concat(propPath, ['$1']), Z.chain(expType.extractors.$1, values)));
        ...
        case BINARY:
          return Z.chain(function (result) {
            ...
            return Z.map(function (result) {
              ...
              return {
                typeVarMap: result.typeVarMap,
                types: Z.lift2(fromBinaryType(expType), or($1s, [expType.types.$1]), or($2s, [expType.types.$2]))
              };
            }, recur(env, typeInfo, result.typeVarMap, expType.types.$2, index, Z.concat(propPath, ['$2']), Z.chain(expType.extractors.$2, values)));
          }, recur(env, typeInfo, typeVarMap, expType.types.$1, index, Z.concat(propPath, ['$1']), Z.chain(expType.extractors.$1, values)));

        case RECORD:
          return Z.reduce(function (e, k) {
            return Z.chain(function (r) {
              return recur(env, typeInfo, r.typeVarMap, expType.types[k], index, Z.concat(propPath, [k]), Z.chain(expType.extractors[k], values));
            }, e);
          }, Right({
            typeVarMap: typeVarMap,
            types: [expType]
      ...
      return function (url) {
        return function (supertypes) {
          return function (test) {
            return function (_1) {
              return function ($1) {
                return _Type(UNARY, name, url, 1, null, supertypes, K(test), [['$1', _1, $1]]);
              };
            };
          };
        };
    ...
    function fromUnaryType(t) {
      return UnaryType(t.name)(t.url)(t.supertypes)(t._test([]))(t._extractors.$1);
          ...
          return function (test) {
            return function (_1) {
              return function (_2) {
                return function ($1) {
                  return function ($2) {
                    return _Type(BINARY, name, url, 2, null, supertypes, K(test), [['$1', _1, $1], ['$2', _2, $2]]);
                  };
                };
              };
            };
    ...
    function fromBinaryType(t) {
      return BinaryType(t.name)(t.url)(t.supertypes)(t._test([]))(t._extractors.$1)(t._extractors.$2);
    ...
    function RecordType(fields) {
      ...
      function format(outer, inner) {
        ...
        var reprs = Z.map(function (k) {
          var t = fields[k];
          return outer(' ') + outer(/^(?!\d)[$\w]+$/.test(k) ? k : show(k)) + outer(' :: ') + inner(k)(show(t));
        }, keys);
          ...
          return function (fields) {
            ...
            function test(env) {
              ...
              return function (x) {
                ...
                return isEmpty(missing) && keys.every(function (k) {
            ...
            return _Type(RECORD, name, url, 0, format, supertypes, test, tuples);
          };
        };
      };
    ...
    function typeVarPred(arity) {
      ...
      return function (env) {
        ...
        return function (x) {
          var test1 = test2(x);
          return env.some(function (t) {
            return filter(t) && test1(t);
          });
        };
      };
    ...
    function invalidArgumentsCount(typeInfo, index, numArgsExpected, args) {
      return new TypeError(trimTrailingSpaces(q(typeInfo.name) + ' applied to the wrong number of arguments\n\n' + underline(typeInfo, K(K(_)), function (index_) {
        return function (f) {
    ...
    function constraintsRepr(constraints, // :: StrMap (Array TypeClass)
    outer, // :: String -> String
    inner // :: String -> TypeClass -> String -> String
      ...
      return when($reprs.length > 0)(wrap('')(outer(' => ')))(when($reprs.length > 1)(parenthesize(outer))(joinWith(outer(', '), $reprs)));
    ...
    function label(label) {
      return function (s) {
        ...
        return strRepeat(' ', Math.floor(delta / 2)) + label + strRepeat(' ', Math.ceil(delta / 2));
      };
    } //  typeVarNames :: Type -> Array String


    function typeVarNames(t) {
      return Z.concat(t.type === VARIABLE ? [t.name] : [], Z.chain(function (k) {
        return typeVarNames(t.types[k]);
      }, t.keys));
    ...
    function showTypeWith(types) {
      ...
      return function (t) {
        ...
        return when(t.type === FUNCTION)(parenthesize(I))(show(t).replace(/\bUnknown\b/g, function () {
          // eslint-disable-next-line no-plusplus
          do var name = String.fromCharCode(code++); while (names.indexOf(name) >= 0);
    ...
    function showValuesAndTypes(env, // :: Array Type
    typeInfo, // :: TypeInfo
    values, // :: Array Any
    pos // :: Integer
    ) {
      ...
      return show(pos) + ')  ' + joinWith('\n    ', Z.map(function (x) {
        return show(x) + ' :: ' + joinWith(', ', or(Z.map(showType, determineActualTypesLoose(env, [x])), ['(no types)']));
      }, values));
    } //  typeSignature :: TypeInfo -> String


    function typeSignature(typeInfo) {
      return typeInfo.name + ' :: ' + constraintsRepr(typeInfo.constraints, I, K(K(I))) + joinWith(' -> ', Z.map(showTypeWith(typeInfo.types), typeInfo.types));
    } //  _underline :: ... -> String


    function _underline(t, // :: Type
    propPath, // :: PropPath
    formatType3 // :: Type -> Array String -> String -> String
    ) {
      return formatType3(t)(propPath)(t.format(_, function (k) {
        return K(_underline(t.types[k], Z.concat(propPath, [k]), formatType3));
      }));
    } //  underline :: ... -> String


    function underline(typeInfo, // :: TypeInfo
    underlineConstraint, // :: String -> TypeClass -> String -> String
    formatType5 // :: Integer -> (String -> String) -> Type -> PropPath -> String -> String
    ) {
      var st = typeInfo.types.reduce(function (st, t, index) {
        var f = B(when(t.type === FUNCTION)(parenthesize(_)))(B(function (f) {
          return _underline(t, [], f);
        })(formatType5(index)));
        st.carets.push(f(r('^')));
      ...
      return typeSignature(typeInfo) + '\n' + _(typeInfo.name + ' :: ') + constraintsRepr(typeInfo.constraints, _, underlineConstraint) + joinWith(_(' -> '), st.carets) + '\n' + _(typeInfo.name + ' :: ') + constraintsRepr(typeInfo.constraints, _, K(K(_))) + joinWith(_(' -> '), st.numbers) + '\n';
    } //  resolvePropPath :: (Type, Array String) -> Type


    function resolvePropPath(t, propPath) {
      return Z.reduce(function (t, prop) {
        return t.types[prop];
      }, t, propPath);
    } //  formatType6 ::
    ...
    function see(label, record) {
      return record.url == null || record.url === '' ? '' : '\nSee ' + record.url + ' for information about the ' + record.name + ' ' + label + '.\n';
    } //  typeClassConstraintViolation :: ... -> Error


    function typeClassConstraintViolation(env, // :: Array Type
    typeInfo, // :: TypeInfo
    typeClass, // :: TypeClass
    index, // :: Integer
    propPath, // :: PropPath
    ...
    typeVarMap // :: TypeVarMap
      ...
      return new TypeError(trimTrailingSpaces('Type-class constraint violation\n\n' + underline(typeInfo, function (tvn) {
        return function (tc) {
          return tvn === expType.name && tc.name === typeClass.name ? r('^') : _;
        };
      }, formatType6(Z.concat([index], propPath))) + '\n' + showValuesAndTypes(env, typeInfo, [value], 1) + '\n\n' + q(typeInfo.name) + ' requires ' + q(expType.name) + ' to satisfy the ' + stripNamespace(typeClass) + ' type-class constraint; ' + 'the value at position 1 does not.\n' + see('type class', typeClass)));
    } //  typeVarConstraintViolation :: ... -> Error


    function typeVarConstraintViolation(env, // :: Array Type
    typeInfo, // :: TypeInfo
    index, // :: Integer
    propPath, // :: PropPath
    valuesByPath // :: StrMap (Array Any)
      ...
      var key = JSON.stringify(Z.concat([index], propPath));
      var values = valuesByPath[key]; //  Note: Sorting these keys lexicographically is not "correct", but it
      //  does the right thing for indexes less than 10.

      var keys = Z.filter(function (k) {
        var values_ = valuesByPath[k];
        return (//  Keep X, the position at which the violation was observed.
          k === key || //  Keep positions whose values are incompatible with the values at X.
          isEmpty(determineActualTypesStrict(env, Z.concat(values, values_)))
        );
      }, sortedKeys(valuesByPath));
      var underlinedTypeVars = underlineTypeVars(typeInfo, Z.reduce(function ($valuesByPath, k) {
      ...
      return new TypeError(trimTrailingSpaces('Type-variable constraint violation\n\n' + underlinedTypeVars + '\n' + Z.reduce(function (st, k) {
        ...
        return isEmpty(values) ? st : {
          idx: st.idx + 1,
          s: st.s + showValuesAndTypes(env, typeInfo, values, st.idx + 1) + '\n\n'
        };
      }, {
        idx: 0,
        s: ''
      }, keys).s + 'Since there is no type of which all the above values are ' + 'members, the type-variable constraint has been violated.\n'));
    } //  invalidValue :: ... -> Error


    function invalidValue(env, // :: Array Type
    typeInfo, // :: TypeInfo
    index, // :: Integer
    propPath, // :: PropPath
    value // :: Any
      ...
      var underlinedTypeVars = underline(typeInfo, K(K(_)), formatType6(Z.concat([index], propPath)));
      return new TypeError(trimTrailingSpaces(t.type === VARIABLE && isEmpty(determineActualTypesLoose(env, [value])) ? 'Unrecognized value\n\n' + underlinedTypeVars + '\n' + showValuesAndTypes(env, typeInfo, [value], 1) + '\n\n' + toMarkdownList('The environment is empty! ' + 'Polymorphic functions require a non-empty environment.\n', 'The value at position 1 is not a member of any type in ' + 'the environment.\n\n' + 'The environment contains the following types:\n\n', showTypeWith(typeInfo.types), env) : // else
      'Invalid value\n\n' + underlinedTypeVars + '\n' + showValuesAndTypes(env, typeInfo, [value], 1) + '\n\n' + 'The value at position 1 is not a member of ' + q(show(t)) + '.\n' + see(arityGte(1)(t) ? 'type constructor' : 'type', t)));
    ...
    function invalidArgumentsLength(typeInfo, // :: TypeInfo
    index, // :: Integer
    numArgsExpected, // :: Integer
    args // :: Array Any
    ) {
      return new TypeError(trimTrailingSpaces(q(typeInfo.name) + ' applied ' + q(show(typeInfo.types[index])) + ' to the wrong number of arguments\n\n' + underline(typeInfo, K(K(_)), function (index_) {
        return function (f) {
          return function (t) {
            return function (propPath) {
              return function (s) {
                return index_ === index ? t.format(_, function (k) {
                  return k === '$1' ? f : _;
                }) : _(s);
              };
            };
    ...
    function withTypeChecking(env, // :: Array Type
    typeInfo, // :: TypeInfo
    impl // :: Function
    ) {
      var n = typeInfo.types.length - 1; //  wrapFunctionCond :: (TypeVarMap, Integer, a) -> a

      function wrapFunctionCond(_typeVarMap, index, value) {
        var expType = typeInfo.types[index];
        if (expType.type !== FUNCTION) return value; //  checkValue :: (TypeVarMap, Integer, String, a) -> Either (() -> Error) TypeVarMap

        function checkValue(typeVarMap, index, k, x) {
          var propPath = [k];
          var t = expType.types[k];
          return t.type === VARIABLE ? Z.chain(function (typeVarMap) {
            return isEmpty(typeVarMap[t.name].types) ? Left(function () {
              return typeVarConstraintViolation(env, typeInfo, index, propPath, typeVarMap[t.name].valuesByPath);
            }) : Right(typeVarMap);
          }, Right(updateTypeVarMap(env, typeVarMap, t, index, propPath, [x]))) : // else
          Z.map(function (r) {
            return r.typeVarMap;
          }, satisfactoryTypes(env, typeInfo, typeVarMap, t, index, propPath, [x]));
        }

        var typeVarMap = _typeVarMap;
        return function (x) {
          if (arguments.length !== expType.arity - 1) {
            throw invalidArgumentsLength(typeInfo, index, expType.arity - 1, slice.call(arguments));
          }

          var args = arguments;
          typeVarMap = assertRight(init(expType.keys).reduce(function (either, k, idx) {
            var arg = args[idx];
      ...
      function wrapNext(_typeVarMap, _values, index) {
        return function (x) {
          ...
          var typeVarMap = assertRight(satisfactoryTypes(env, typeInfo, _typeVarMap, typeInfo.types[index], index, [], args)).typeVarMap;
          ...
          if (index + 1 === n) {
            ...
            typeVarMap = assertRight(satisfactoryTypes(env, typeInfo, typeVarMap, typeInfo.types[n], n, [], [value])).typeVarMap;
      ...
      var wrapped = typeInfo.types[0].type === NO_ARGUMENTS ? function () {
        if (arguments.length !== 0) {
          throw invalidArgumentsCount(typeInfo, 0, 0, slice.call(arguments));
        }

        var value = impl();
        var typeVarMap = assertRight(satisfactoryTypes(env, typeInfo, {}, typeInfo.types[n], n, [], [value])).typeVarMap;
        return wrapFunctionCond(typeVarMap, n, value);
      } : wrapNext({}, [], 0);
    ...
    var defTypes = [String_, StrMap(Array_(TypeClass)), NonEmpty(Array_(Type)), AnyFunction, AnyFunction];
        ...
        return function (constraints) {
          return function (expTypes) {
            return function (impl) {
              return opts.checkTypes ? withTypeChecking(opts.env, {
                name: name,
                constraints: constraints,
                types: expTypes.length === 1 ? Z.concat([NoArguments], expTypes) : expTypes
              }, impl) : impl;
            };
          };
        };
    ...
    return {
      ...
      Function: def('Function')({})([NonEmpty(Array_(Type)), Type])(Function_),
      ...
      Unknown: Unknown,
      env: env,
      create: def('create')({})([RecordType({
        checkTypes: Boolean_,
        env: Array_(Type)
      }), Unchecked(joinWith(' -> ', Z.map(show, defTypes)))])(create),
      test: def('test')({})([Array_(Type), Type, Any, Boolean_])(test),
      NullaryType: def('NullaryType')({})([String_, String_, Array_(Type), Unchecked('(Any -> Boolean)'), Type])(NullaryType),
      UnaryType: def('UnaryType')({
        f: [Z.Foldable]
      })([String_, String_, Array_(Type), Unchecked('(Any -> Boolean)'), Unchecked('(t a -> f a)'), Unchecked('Type -> Type')])(function (name) {
        return B(B(B(B(def(name)({})([Type, Type])))))(UnaryType(name));
      }),
      BinaryType: def('BinaryType')({
        f: [Z.Foldable]
      })([String_, String_, Array_(Type), Unchecked('(Any -> Boolean)'), Unchecked('(t a b -> f a)'), Unchecked('(t a b -> f b)'), Unchecked('Type -> Type -> Type')])(function (name) {
        return B(B(B(B(B(def(name)({})([Type, Type, Type]))))))(BinaryType(name));
      }),
      EnumType: def('EnumType')({})([String_, String_, Array_(Any), Type])(EnumType),
      RecordType: def('RecordType')({})([StrMap(Type), Type])(RecordType),
      NamedRecordType: def('NamedRecordType')({})([NonEmpty(String_), String_, Array_(Type), StrMap(Type), Type])(NamedRecordType),
      TypeVariable: def('TypeVariable')({})([String_, Type])(TypeVariable),
      UnaryTypeVariable: def('UnaryTypeVariable')({})([String_, Unchecked('Type -> Type')])(function (name) {
        return def(name)({})([Type, Type])(UnaryTypeVariable(name));
      }),
      BinaryTypeVariable: def('BinaryTypeVariable')({})([String_, Unchecked('Type -> Type -> Type')])(function (name) {
        return def(name)({})([Type, Type, Type])(BinaryTypeVariable(name));
      }),
      Thunk: def('Thunk')({})([Type, Type])(Thunk),
  ...
  (function (f) {
    ...
    if (typeof __doctest !== 'undefined') {
      ...
      var S = function () {
        ...
        var MaybeType = $.UnaryType('sanctuary-maybe/Maybe')('')(function (x) {
    ...
    var Nothing = Maybe.Nothing = Object.create(Nothing$prototype); //# Maybe.Just :: a -> Maybe a
    //.
    ...
    Maybe['@@type'] = 'sanctuary-maybe/Maybe@1'; //# Maybe.fantasy-land/empty :: () -> Maybe a
    //.
  ...
  (function (f) {
    ...
    if (typeof __doctest !== 'undefined') {
      ...
      var S = function () {
        ...
        var PairType = $.BinaryType('sanctuary-pair/Pair')('')(function (x) {
        ...
        var env = Z.concat(S.env, [$.TypeClass, PairType($.Unknown)($.Unknown)]);
        return S.create({
          checkTypes: true,
          env: env
        });
    ...
    Pair['@@type'] = 'sanctuary-pair/Pair@1'; //# Pair#@@show :: (Showable a, Showable b) => Pair a b ~> () -> String
    //.
    ...
    function Pair$prototype$lte(other) {
      return Z.equals(this.fst, other.fst) ? Z.lte(this.snd, other.snd) : Z.lte(this.fst, other.fst);
...
var sanctuary = createCommonjsModule(function (module) {
  ...
  (function (f) {
    /* istanbul ignore else */
    {
      module.exports = f(sanctuaryDef, sanctuaryEither, sanctuaryMaybe, sanctuaryPair, sanctuaryShow, sanctuaryTypeClasses, sanctuaryTypeIdentifiers);
    }
    ...
    if (typeof __doctest !== 'undefined') {
      ...
      var S = function (S) {
        var S_ = S.create({
          checkTypes: true,
          env: S.env.concat([__doctest.require('./test/internal/List').Type($.Unknown), Sum.Type])
        });
    ...
    var Right = Either.Right; //  Nothing :: Maybe a

    var Nothing = Maybe.Nothing; //  Just :: a -> Maybe a

    var Just = Maybe.Just; //  B :: (b -> c) -> (a -> b) -> a -> c
    ...
    var r = $.TypeVariable('r'); //  :: Type -> Type

    var f = $.UnaryTypeVariable('f');
    var m = $.UnaryTypeVariable('m');
    var t = $.UnaryTypeVariable('t');
    var w = $.UnaryTypeVariable('w'); //  :: Type -> Type -> Type

    var p = $.BinaryTypeVariable('p');
    var s = $.BinaryTypeVariable('s'); //  TypeRep :: Type -> Type

    var TypeRep = $.UnaryType('TypeRep')('https://github.com/fantasyland/fantasy-land#type-representatives')([])(function (x) {
      return $.test([])($.AnyFunction)(x) || x != null && $.test([])($.String)(x['@@type']);
    })(K([])); //  Options :: Type
    ...
    function create(opts) {
      ...
      S.unchecked = opts.checkTypes ? create({
        checkTypes: false,
        env: opts.env
      }) : S;
      return S;
    ...
    function last(foldable) {
      //  Fast path for arrays.
      if (Array.isArray(foldable)) {
        return foldable.length > 0 ? Just(foldable[foldable.length - 1]) : Nothing;
      }
    ...
    function _takeDrop(arrayCase, generalCase) {
      return function (n) {
        return function (xs) {
          if (n < 0) return Nothing; //  Fast path for arrays.
    ...
    var take = _takeDrop(function (n, xs) {
      return xs.slice(0, n);
    }, function (n, xs, x) {
      return n > 0 ? Z.append(x, xs) : xs;
    });
    ...
    var drop = _takeDrop(function (n, xs) {
      return xs.slice(n);
    }, function (n, xs, x) {
      return n > 0 ? xs : Z.append(x, xs);
    });
    ...
    _.sort = {
      consts: {
        a: [Z.Ord],
        m: [Z.Applicative, Z.Foldable, Z.Monoid]
      },
      types: [m(a), m(a)],
      impl: Z.sort
    ...
    function prop(key) {
      return function (x) {
        ...
        throw new TypeError('‘prop’ expected object to have a property named ' + '‘' + key + '’; ' + show(x) + ' does not');
      };
    }
    ...
    function props(path) {
      return function (x) {
        return path.reduce(function (x, key) {
          var obj = toObject(x);
          if (key in obj) return obj[key];
          throw new TypeError('‘props’ expected object to have a property at ' + show(path) + '; ' + show(x) + ' does not');
        }, x);
      };
    }
    ...
    function value(key) {
      return function (strMap) {
        return Object.prototype.propertyIsEnumerable.call(strMap, key) ? Just(strMap[key]) : Nothing;
      };
    }
    ...
    function mean(foldable) {
      var result = Z.reduce(function (acc, n) {
        acc.total += n;
        acc.count += 1;
        return acc;
      }, {
    ...
    var validFloatRepr = new RegExp('^' + // start-of-string anchor
    '\\s*' + // any number of leading whitespace characters
    '[+-]?' + // optional sign
    requiredNonCapturingGroup(['Infinity', // "Infinity"
    'NaN', // "NaN"
    requiredNonCapturingGroup(['[0-9]+', // number
    '[0-9]+[.][0-9]+', // number with interior decimal point
    '[0-9]+[.]', // number with trailing decimal point
    '[.][0-9]+' // number with leading decimal point
    ]) + optionalNonCapturingGroup(['[Ee]' + // "E" or "e"
    '[+-]?' + // optional sign
    '[0-9]+' // exponent
    ...
    var Radix = $.NullaryType('Radix')('')([$.Integer])(function (x) {
      return x >= 2 && x <= 36;
    ...
    function match(pattern) {
      return function (s) {
        return Z.map(toMatch, Z.reject(equals(null), Just(s.match(pattern))));
      };
    }
    ...
    function words(s) {
      ...
      return words.slice(words[0] === '' ? 1 : 0, words[len - 1] === '' ? len - 1 : len);
    }
    ...
    return create({
      checkTypes: typeof process === 'undefined' || process == null || process.env == null || process.env.NODE_ENV !== 'production',
      env: $.env
    });
...
function pipecanvas() {
  var fcontext = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
  var pipegraph = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  ...
  var COMMON_INPUT = (idpart, output) => {
    // typing might contain other attributes, like payable
    return {
  ...
  function getRandomColor() {
    ...
    var getPartial = letters => letters[Math.floor(Math.random() * letters.length)];
  ...
  function typeOptions(type) {
    ...
    var typedef = pipe1.types[type];
    var similar = Object.keys(pipeopts.types).find(key => typedef === pipe1.types[key]);
    ...
    while (pipeopts.typecolors[color] && end < 400) {
      color = (_readOnlyError("color"), getRandomColor());
      end += (_readOnlyError("end"), 1);
    }
  ...
  var coloropts = Object.assign({}, DEFAULT_OPTIONS.colors, options.colors || {});
  var pipeopts = Object.assign({}, DEFAULT_OPTIONS, options, {
    colors: coloropts
  });
  var current_stage = {
    settings: {
  ...
  var targets = JSON.parse(JSON.stringify(DEFAULT_TARGETS));
  var pipe1 = new pipe(pipetypes);
  ...
  function runnable(graph) {
    ...
    var rich = pipe1.enrich_graph(pipe1.get_indexed_func())(graph);
    console.log('runnable rich', rich);
    var runtime = pipe1.make_runtime(pipe1.get_indexed_func())(rich);
    current_stage.settings.r_graph = runtime;
    return runtime;
  }

  function dstart() {
    var coords = JSON.parse(JSON.stringify(d3.mouse(this))); // d3.event.x, d3.event.y

    var coordp = {
      x: coords[0],
      y: coords[1]
    };
    var done = false; // console.log("srttt", coords)
    // console.log('targets', targets)

    for (var key in targets.for_click_edge) {
      var value = targets.for_click_edge[key];

      if (value.l <= coordp.x && value.r >= coordp.x) {
        if (value.t <= coordp.y && value.b >= coordp.y) {
          console.log("edge-click", value, key);
          var new_gr = JSON.parse(JSON.stringify(current_stage.settings.r_graph.rich_graph.init));
          new_gr = pipe1.remove_edge(new_gr)(JSON.parse("[" + key + "]"));
          new_gr = runnable(new_gr); // console.log( "dstart new_gryyy", new_gr)
    ...
    for (var _key in targets.for_click) {
      var _value = targets.for_click[_key]; // console.log("sttt", value, coordp.x, coordp.y);

      if (_value.l <= coordp.x && _value.r >= coordp.x) {
        if (_value.t <= coordp.y && _value.b >= coordp.y) {
          //mutable current_edge = {pos:[coords[0], coords[1], coords[0], coords[1]]}
          // console.log("starttt-click", value, key)
          var key1 = JSON.parse("[" + _key + "]");

          if (key1[1] === 1) {
            console.log("clicked node", _value, key1); //return change_mode(key1[0]);
          }

          if (key1[1] === 0) {
            //console.log( "new_gr",current_stage)
            var _new_gr = JSON.parse(JSON.stringify(current_stage.settings.r_graph.rich_graph.init));

            _new_gr = pipe1.remove_node(_new_gr)(key1[0]);
            _new_gr = runnable(_new_gr); // console.log("dstart new_gryyy", new_gr)

            current_stage.settings.r_graph = _new_gr;
            done = true;
          }
        }
      }
    ...
    for (var _key2 in targets.for_drag) {
      var _value2 = targets.for_drag[_key2];

      if (_value2.l <= coordp.x && _value2.r >= coordp.x) {
        if (_value2.t <= coordp.y && _value2.b >= coordp.y) {
          current_edge.pos = [(_value2.r + _value2.l) / 2, (_value2.b + _value2.t) / 2, coords[0], coords[1]];
          current_edge.source = _value2.port; // console.log("dstart starttt-edge", value)
        }
      }
  ...
  function ddrag(e) {
    ...
    if (current_edge.pos.length === 4) {
      var co = JSON.parse(JSON.stringify(current_edge.pos));
      ...
      drawExactArrow(current_stage.settings.ctx, current_edge.pos[0], current_edge.pos[1], current_edge.pos[2], current_edge.pos[3], current_edge.source.type, 1, true); // console.log('ddrag current_edge', current_edge)
    }
  }
  ...
  function nearestf(pos) {
    ...
    for (var rec in targets.end_drag) {
      ...
      var targetIns = current_stage.settings.r_graph.rich_graph.n[targets.end_drag[rec].port.i].in[targets.end_drag[rec].port.port];

      if (pipe1.types[current_edge.source.type] === pipe1.types[targets.end_drag[rec].port.type] && // don't connect to the same input port:
      targetIns[0] !== current_edge.source.i && // only ports connected to input ports
      targetIns[0] > 2999) {
        acc = nearest ? nearest : targets.end_drag[rec];
        d1 = dist(acc, pos); // console.log('nearestf', targets.end_drag[rec], pos)

        d2 = dist(targets.end_drag[rec], pos); // console.log('nearestf', d1,d2)

        if (d1 >= d2) {
          if (d2 < 30000) {
            nearest = targets.end_drag[rec];
          } else {
            nearest = false; // console.log('nearestf', d2)
          }
        }
      }
    }
  ...
  function add_edge_to_input(current_edge) {
    ...
    var n1 = r_graph.rich_graph.n[current_edge.source.i];
    var new_gr = JSON.parse(JSON.stringify(r_graph.rich_graph.init));
    var sourceid = r_graph.rich_graph.n[current_edge.source.i].id;
    var inputIdx = current_edge.source.port - 1;
    var sourceInput = r_graph.context[sourceid].pfunction.gapi.outputs[inputIdx];
    var newnode = COMMON_INPUT(current_edge.source.i, {
      name: sourceInput.name,
      type: sourceInput.type
    });
    pipe1.add_indexed_func(newnode);
    new_gr = pipe1.add_node(new_gr)({
      i: current_edge.source.i,
      id: newnode._id
    });

    for (var e in r_graph.rich_graph.e) {
      if (r_graph.rich_graph.e[e][0] === current_edge.source.i) new_gr = pipe1.add_edge(new_gr)(r_graph.rich_graph.e[e]);
    }
  ...
  function graph_edit(ctx, r_graph) {
    var dnodes = {};
    var rows = r_graph.runnable_graph.length;
    var cols = 0;

    for (var nodes of r_graph.runnable_graph) {
      for (var node of nodes) {
        var node_i = r_graph.rich_graph.n[node];
        var node_x = r_graph.context[node_i.id];
        var len = Math.max(node_x.pfunction.gapi.inputs.length, node_x.pfunction.gapi.outputs.length);
        dnodes[node] = {
          x: node_i.position.x,
          y: node_i.position.y,
          container: node_x.pclass.name,
          name: node_x.pfunction.gapi.name,
          len: len,
          context: node_x.pfunction.gapi,
          i: node
        };
    ...
    current_stage.settings = {
      width: pipeopts.width,
      height: pipeopts.height,
      nodes: dnodes,
      edges: r_graph.rich_graph.e,
      matrix: [rows, cols],
      transform: {
        x: 0,
        y: 0,
  ...
  function pos_calculate(x, y, l, matrix, dims) {
    var marginw = pipeopts.width / 30;
    var marginh = 0;
    var minh = 16;
    var stepw = (dims[0] - 2 * marginw) / (matrix[1] + 0);
    var steph = (dims[1] - 2 * marginh + minh) / (matrix[0] - 1); //-minh/2

    return {
      x: x * stepw + marginw + l * stepw / 2,
      y: y * steph - minh / 2,
      l: l * stepw,
      h: minh,
      steps: {
  ...
  function graph_show(ctx2, width, height, nodes, edges, matrix, transform, r_graph) {
    ...
    for (var edge of edges) {
      var e = edge;
      var p1 = ports_pos[[e[0], e[1], 1]];
      var p2 = ports_pos[[e[2], e[3], 0]];
      var mid = [(p1.x + p2.x) / 2, (p1.y + p2.y) / 2];
      var r = 4; //console.log(edge, p1,p2)

      drawExactArrow(ctx2, p1.x, p1.y, p2.x, p2.y, p1.type, 1);
      targets.for_click_edge[e] = {
        t: mid[1] - r,
        b: mid[1] + r,
        l: mid[0] - r,
        r: mid[0] + r
  ...
  function node_show(ctx, node, matrix, dims, ports_pos) {
    ...
    var x = node.x;
    var y = node.y;
    var pos = pos_calculate(x, y, node.len, matrix, dims);
    var nw = pos.l;
    ports_show(ctx, pos, node.context, ports_pos, node.i);
    var bodycol = pipeopts.bodyColorFunction ? pipeopts.bodyColorFunction(node.context) : null;
    ctx.fillStyle = bodycol ? bodycol : pipeopts.colors.nodeBody;
    rect1([ctx, pos.x, pos.y, nw, pos.h, pos.h / 2]);
    targets.for_click[[node.i, 1]] = {
      l: pos.x - nw / 2,
      r: pos.x + nw / 2 - 16,
      t: pos.y - pos.h / 2,
      b: pos.y + pos.h / 2
    ...
    ctx.beginPath();
    ctx.textAlign = "center"; //ctx.font = (18*scale-9)+"px "+pipeopts.font
    //context.lineJoin = "round";

    ctx.fillStyle = pipeopts.colors.text; //ctx.fillText(shorten(node.container,max*5+2), pos.x, pos.y-2 );
  ...
  function ports_show(ctx, pos, node_x, ports_pos, i) {
    ...
    for (var port1 of node_x.inputs) {
      ...
      ctx.restore();
      var xt = pos.x - pos.l / 2 + pos.steps.w * (ndx + 0.5);
      var yt = pos.y - pos.h / 2;
      ctx.save();
      ctx.translate(xt, yt);
      ctx.rotate(-Math.PI / 4);
      ctx.font = 8 / 1 + 4 + "px " + pipeopts.font;
      ...
      if (parseInt(i) < 3000) targets.end_drag[[i, ndx + 1]] = {
        x: x,
        y: y,
        port: {
          i: i,
    ...
    for (var _port of node_x.outputs) {
      //console.log(port1)
      ctx.fillStyle = typeOptions(_port.type).color;

      var _x = pos.x - pos.l / 2 + pos.steps.w * (ndx + 0.5);

      var _y = pos.y + pos.h / 2;

      rect1([ctx, _x, _y, nw, nw, 0, 0, 10, 10]);
      ports_pos[[i, ndx + 1, 1]] = {
        x: _x,
        y: _y,
        type: _port.type
      };
      targets.for_drag[[i, ndx + 1]] = {
        l: _x - 10,
        r: _x + 10,
        t: _y,
        b: _y + 10,
  ...
  function shorten(txt, noChar) {
    ...
    var half = Math.floor(noChar / 2);
    return txt.slice(0, half) + ".." + txt.slice(-half);
  }

  function drawExactArrow(ctx2, x1, y1, x2, y2, type, scale) {
    var temporary = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
    var midx = (x2 + x1) / 2;
    var midy = (y2 + y1) / 2;
    var context = ctx2; //let color  = types[type].color

    var srt = "M" + x1 + "," + y1 + "C" + x1 + "," + midy + "," + midx + "," + midy + "," + midx + "," + midy + "C" + midx + "," + midy + "," + x2 + "," + midy + "," + x2 + "," + y2; //let scale = 1
    ...
    context.closePath(); //context.moveTo(midx, midy)
  ...
  function redraw() {
    ...
    if (onchangeCallb) {
      var graph = JSON.parse(JSON.stringify(current_stage.settings.r_graph));
      onchangeCallb(graph);
    }
  }
  ...
  function showGraph() {
    var canvas1 = d3.select(pipeopts.domid).attr('width', pipeopts.width).attr('height', pipeopts.height);
    var ctx1 = canvas1.node().getContext('2d');
  ...
  function addFunction(pfunction, index) {
    ...
    pipe1.add_indexed_func(pfunction); // console.log(current_stage.settings.r_graph);

    var lastIndex = Math.max(100, ...Object.keys(current_stage.settings.r_graph.rich_graph.init.n).filter(key => key < 3000)); // console.log('lastIndex', lastIndex);
  ...
  function clearGraph() {
    var graph = Object.assign({}, DEFAULT_GRAPH);
    current_stage.settings.r_graph = runnable(graph);
    showGraph();
  }

  function setGraph(pipegraph) {
    var fcontext = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

    if (!pipegraph || !(pipegraph instanceof Object) || Object.keys(pipegraph).length === 0) {
      pipegraph = Object.assign({}, DEFAULT_GRAPH);
    }

    var mergedContext = Object.assign({}, pipe1.get_indexed_func(), fcontext);
    pipe1.set_indexed_func(mergedContext);
    pipegraph.e.forEach(edge => {
      if (edge[0] > 2999) {
        var sourceOutput = pipe1.get_indexed_func()[pipegraph.n[edge[2]].id].pfunction.gapi.inputs[edge[3] - 1];
        var newnode = COMMON_INPUT(edge[0], {
        ...
        if (!pipegraph.n[edge[0]]) {
          pipegraph = pipe1.add_node(pipegraph)({
            i: edge[0],
            id: newnode._id
          });
        }
